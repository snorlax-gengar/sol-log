...
/**
 * 직전에 저장한 젖병(분유/유축 모유/이유식) ml을 불러온다.
...
export function loadLastFeedingPreset() {
  try {
    ...
    const pumpedMl = toPositiveMl(parsed.pumpedMl)
    const foodMl = toPositiveMl(parsed.foodMl)
    if (formulaMl === 0 && pumpedMl === 0 && foodMl === 0) return null

    return { formulaMl, pumpedMl, foodMl }
  } catch {
    return null
  }
}
...
export function saveLastFeedingPreset(form) {
  try {
    ...
    if (formulaMl === 0 && pumpedMl === 0 && foodMl === 0) return

    window.localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ formulaMl, pumpedMl, foodMl }),
    )
  } catch {
